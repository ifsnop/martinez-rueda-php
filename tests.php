...
$test = [
	...
	8 => [
		...
		'res' => [
			'union' => [[[[150.867925,-10.013013],[150.870880365372,-10.016787342292],[150.87071943283,-10.01682719716],[150.873,-10.017],[150.867925,-10.013013]]]],
		],
	],
	9 => [
	...
	13 => [ // ejemplo de fallo para TOLERANCE == 1e-9, pero que se soluciona con TOLERANCE == 1e-12 (son dos polígonos con un vértice casi colineal, pero no exactamente, y el algoritmo de intersección los considera paralelos si la tolerancia es demasiado alta)
		'region_a' => [[
    		[40.53363568130732, -3.5245698583174576],
    		[40.53363568130732, -3.523313475015876 ],
    		[40.49956209027567, -3.523761811338624 ],
    		[40.49956209027567, -3.5241215219947093],
		]],
		'region_b' => [[
			[40.498860930663234, -3.523769587464169],
    		[40.49886780983072,  -3.524147669692794],
    		[40.4995689976698,   -3.524121458419516],
    		[40.49956237672506,  -3.523761808702163],
		]],
		'res' => [
			'union' =>[[
				[40.49956209027566700, -3.52376181133862421],
				[40.53363568130731664, -3.52331347501587588],
				[40.53363568130731664, -3.52456985831745762],
				[40.49956594138672017, -3.52412157266717774],
				[40.49886780983072043, -3.52414766969279381],
				[40.49886093066323411, -3.52376958746416902],
			]],
		],
	],
];
...
print "xoring australia with shifted australia...";
$australia = MR\GJTools::geojsonToArray("tests/continents_australia.json");
$polygon_australia = MR\Polygon::create()->fillFromArray($australia);
...
// exit(1);
if ($result->numPoints == 1340) print "PASS" . PHP_EOL;
else print "FAIL(" . $result->numPoints . ")" . PHP_EOL;

print "xoring europe with shifted europe...";
$europe = MR\GJTools::geojsonToArray("tests/continents_europe.json");
$polygon_europe = MR\Polygon::create()->fillFromArray($europe);
$shifted_europe = displaceMultiPolygon($europe, -0.5);
$shifted_polygon_europe = MR\Polygon::create()->fillFromArray($shifted_europe);
$result = MR\Algorithm::xoring($polygon_europe, $shifted_polygon_europe);
if ($result->numPoints == 9178) print "PASS" . PHP_EOL;
else print "FAIL(" . $result->numPoints . ")" . PHP_EOL;
